        ...
        >
          <div className="p-6 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge variant={getBadgeVariant(course.type) as any} className="mb-2">
                  {course.type.replace("_", " ")}
                </Badge>
                <h3 className="text-xl font-semibold group-hover:text-primary transition-colors">{course.name}</h3>
              </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="h-2 w-full rounded-full bg-secondary/70">
                  <div
                    className={`h-2 rounded-full ${getProgressColor(course.progressPercentage)}`}
              ...
              <div className="pt-2 border-t border-border/50">
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      {course.totalProjects} Project{course.totalProjects !== 1 ? "s" : ""}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {course.lastReviewDate && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>Last reviewed: {new Date(course.lastReviewDate).toLocaleDateString()}</span>
                </div>
              )}
            </div>
          </div>
