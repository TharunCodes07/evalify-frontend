  ...
  hooks: {
    async authenticate(page) {
      ...
      page.on("requestfailed", (r) =>
        console.log("[net-fail]", r.failure()?.errorText, r.url())
      );
      page.on(
        "console",
        (m) =>
      ...
      try {
        ...
        if (!clicked)
          console.log(
            "[Auth] No explicit login button; assuming auto-redirect."
          );
      } catch (e) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const seen = await Promise.race([
          f
            .waitForSelector(
              '#username, input[name="username"], input#email, input[name="email"]',
              { timeout: 5000 }
            )
            .then(() => true)
            .catch(() => false),
          f
            .waitForSelector(
              '#kc-login, button[name="login"], input[name="login"], button[type="submit"]',
              { timeout: 5000 }
            )
            .then(() => true)
            .catch(() => false),
      ...
      try {
        ...
        await page.screenshot({
          path: "auth-failed-before-submit.png",
          fullPage: true,
        });
        throw new Error(
          "Could not find username/password inputs. See auth-failed-before-submit.png"
        );
      }
      ...
      const submitSel =
        '#kc-login, button[name="login"], input[name="login"], button[type="submit"]';
      const appUrlReached = () =>
        page.url().startsWith(APP) && !/\/login\b/.test(page.url());
      ...
      const waitForFinished = Promise.race([
        page
          ...
          .catch(() => null),
        page
          .waitForFunction(appUrlReached, { timeout: 60000 })
          .catch(() => null),
        page
          .waitForResponse(
            (res) => {
              const u = res.url();
              return /openid-connect\/token|redirect_uri|session_state|code=/.test(
                u
              );
            },
            { timeout: 60000 }
      ...
      if (!appUrlReached()) {
        await page
          .waitForNetworkIdle({ idleTime: 800, timeout: 20000 })
          .catch(() => null);
      }
      if (!appUrlReached()) {
        await page.screenshot({
          path: "auth-stuck-after-submit.png",
          fullPage: true,
        });
        throw new Error(
          "Login didn't reach app. See auth-stuck-after-submit.png"
        );
      }
